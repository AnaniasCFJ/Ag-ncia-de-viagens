...
public class Main2 {
    ...
    public static void main(String[] args) {
        ...
        // Armazenar os dados dos clientes e iniciar suas threads
        ...
        // Cria um grupo de clientes associados a uma thread específica

        List<Cliente> grupo = new ArrayList<>();

        for (Cliente cliente : clientes){
            grupo.add(cliente);
            if (grupo.size() == clientes.size()/MAX_THREADS){
        ...
        //escrever os dados no arquivo de saida paralela
        String nomeArquivo = "ArqSaidaParal.txt";
    ...
    public static void processarGrupo(List<Cliente> grupo) {
        Thread thread = new Thread(() -> {
            for (Cliente cliente : grupo) {
                Main2.tarefa(cliente);
            }
        });
        threads.add(thread); 
        thread.start();
    ...
    public static void tarefa(Cliente cliente2){
            synchronized(dados){
                                ...
                                if (hotel1.getDias(i + j) > 0) {
                                    if (j == (t - 1)) {
                                        ...
                                        if (allVoos.size() > 0 && hotel1.getPreco() * t + (Float) allVoos.get(0).get(2) <= cliente2.getOrcamentoMaximo()) {
                                            ...
                                            hotel1.setDias(i, hotel1.getDias(i) - 1);
                                            ...
                                            synchronized(dados){
                                                ...
                                                dados.set(4, (double) dados.get(4) + hotel1.getPreco()); // adiciona ao total gasto em hotéis
                                                dados.set(5, (double) dados.get(5) + precoVooFinal); // adiciona ao total gasto em voos
                                            }
                                        }
